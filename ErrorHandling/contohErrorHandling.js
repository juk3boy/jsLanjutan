...
function getMoveis(keyword) {
    ...
    return fetch('http://www.omdbapi.com/?apikey=bd1aaa80&s=' + keyword)
        // response.json akan menghasilkan Promise
        // awalnya .then(response => response.json())
        .then(response => {
            // console.log(response);

            if (!response.ok) {
                throw new Error(response.statusText);  /** ini kita lempar erorr nya keatas */
            }

            return response.json();
        })
        // .then(response => console.log(response)); /** melihat di console.log ini akan menghasilkan object */
        // .then(response => response.Search);  /** sudah tidak muncul error lg maka tidak langsund ditulis spt ini */

        .then(response => {
            if (response.Response === "False") {
                throw new Error(response.Error);
            }
            /** kalau tidak error maka kita langsung munculkan seacrh nya */
...
function getMovieDetail(imdbid) {
    return fetch('http://www.omdbapi.com/?apikey=bd1aaa80&i=' + imdbid)
        // .then(response => response.json())    /** awalnya spt ini */
